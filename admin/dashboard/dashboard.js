...
(async () => {
  ...
  const { data } = await supabase
    .from('users').select('role, user_id').eq('user_id', user.id).single();
...
async function loadStats() {
  const [
    { count: uc }, { count: tc }, { count: wc }, { count: ic },
    { data: usersChart }
  ] = await Promise.all([
...
function renderChart(users) {
  ...
  const counts = months.map(m =>
    users.filter(u => { const d = new Date(u.created_at); return d.getFullYear()===m.year && d.getMonth()===m.month; }).length
  );
  const max = Math.max(...counts, 1);
  container.innerHTML = counts.map((c, i) => `
    <div class="chart-bar-wrap">
      <div class="chart-bar" style="height:${Math.max((c/max)*140,4)}px" title="${c}명"></div>
...
function goToStep(step) {
  ...
  document.querySelector(`.step-panel[data-step="${step}"]`).classList.add('is-active');
  document.querySelector(`.step-dot[data-step="${step}"]`).classList.add('is-active');
  document.querySelector(`.step-label[data-step="${step}"]`).classList.add('is-active');

  document.getElementById('toolStepPrev').style.display = step > 0 ? 'inline-flex' : 'none';
  document.getElementById('toolStepNext').style.display = step < TOTAL_STEPS - 1 ? 'inline-flex' : 'none';
  document.getElementById('toolModalSave').style.display = step === TOTAL_STEPS - 1 ? 'inline-flex' : 'none';
  currentStep = step;
}
...
function openToolModal(tool = null) {
  const fields = [
    'tool_name','tool_company','tool_cat','tool_subcat','tool_key','icon',
    'tool_des','tool_link','pricing','iframe','tool_prom',
    'plan1_name','plan1_price','plan1_des',
    'plan2_name','plan2_price','plan2_des',
    'plan3_name','plan3_price','plan3_des',
    'tool_plan_etc',
  ];
  ...
  fields.forEach(f => {
    ...
    if (!el) return;
    el.value = tool?.[f] || (f === 'tool_cat' ? 'media' : '');
  });

  classes.forEach(c => {
    const el = document.getElementById(`f_${c}`);
    if (el) el.checked = tool?.[`tool_${c}`] ? true : false;
  });
...
window.openEditTool = async (id) => {
  const { data } = await supabase.from('tools').select('*').eq('tool_ID', id).single();
  openToolModal(data);
};
...
document.getElementById('toolModalSave').addEventListener('click', async () => {
  ...
  const payload = {
    ...
    tool_plan2_price:document.getElementById('f_plan2_price').value.trim(),
    ...
    tool_plan3_price:document.getElementById('f_plan3_price').value.trim(),
    ...
    tool_class1: document.getElementById('f_class1').checked ? document.getElementById('f_class1').value : null,
    tool_class2: document.getElementById('f_class2').checked ? document.getElementById('f_class2').value : null,
    tool_class3: document.getElementById('f_class3').checked ? document.getElementById('f_class3').value : null,
    tool_class4: document.getElementById('f_class4').checked ? document.getElementById('f_class4').value : null,
    tool_class5: document.getElementById('f_class5').checked ? document.getElementById('f_class5').value : null,
  };
...
function renderUsersTable(users) {
  ...
  tbody.innerHTML = users.map(u => `
    <tr>
      ...
      <td><button class="btn-icon" onclick="toggleUserRole('${esc(u.user_id)}','${u.role}')">${u.role === 'admin' ? '권한 해제' : '관리자 지정'}</button></td>
    </tr>`).join('');
}
...
window.openAnswerModal = (id) => {
  ...
  document.getElementById('inquiryUserId').value   = item.user_id;   // ✅ 알림 발송용
...
// ✅ 답변 저장 + notifications 테이블에 알림 INSERT
document.getElementById('inquiryModalSave').addEventListener('click', async () => {
  ...
  // 2. 유저에게 알림 발송 (notifications INSERT)
  if (userId) {
    const { error: notiError } = await supabase
      .from('notifications')
      .insert({
        user_id:      userId,         // 알림 받을 유저
        sender_id:    adminUserId,    // 발신자 (관리자)
        type:         'inquiry',      // alert.js에서 'inquiry' 타입으로 처리
        reference_id: inquiryId,      // 문의 ID
        is_read:      false,
